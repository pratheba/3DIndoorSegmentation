function neighbourFaces = GetNeighbouringFaces(face, edgeList)
    ...
    count = 1;
    for i = 0:2
        neighbourFaces{count} = edgeList([num2str(face(mod(i,3)+1)) '/' num2str(face(mod(i+1,3)+1))]);
        count = count+1;
        neighbourFaces{count} = edgeList([num2str(face(mod(i+1,3)+1)) '/' num2str(face(mod(i,3)+1))]);
        count = count+1;
